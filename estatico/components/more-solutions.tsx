            ...
            return (
              <motion.div
                ...
                className={`rounded-lg shadow-md overflow-hidden transition-all duration-300 ${
                  isCurrent ? " hidden" : "bg-white hover:shadow-xl"
                }`}
                ...
                <div className="p-6 flex flex-col h-full">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${
                      isCurrent ? " hidden" : "bg-primary/10"
                    }`}
                  >
                    <solution.icon className={`${isCurrent ? " hidden" : "text-primary"}`} size={24} />
                  </div>
                  <h3 className={`text-xl font-bold mb-3 ${isCurrent ? " hidden" : ""}`}>{solution.title}</h3>
                  <p className={`flex-grow ${isCurrent ? " hidden" : "text-muted-foreground mb-6"}`}>
